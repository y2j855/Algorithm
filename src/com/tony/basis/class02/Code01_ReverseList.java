...
public class Code01_ReverseList {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < testTime; i++) {
            ...
            Node back1 = testReverseList(reverse1);
            if (!checkListEqual(node1, back1)) {
    ...
    // 要求无环，有环别用这个函数
    private static boolean checkListEqual(Node head1, Node head2) {
        while (head1 != null && head2 != null) {
            if (head1.value != head2.value) {
                return false;
            }
    ...
    private static Node testReverseList(Node head) {
        if (head == null) {
            return null;
        }
    ...
    private static Node generateRandomList(int len, int value) {
        int size = (int) (Math.random() * (len + 1));
        if (size == 0) {
            return null;
        }
